...
//
// 用法（推荐用同目录的 ocr_macos.sh 包装脚本，它会设置好模块缓存路径）:
...
//
// 注意：识别需要访问系统 Vision 服务。在受限沙箱里可能失败，换到普通终端窗口重跑即可。
...
guard !paths.isEmpty else {
    FileHandle.standardError.write("用法: sh scripts/ocr_macos.sh <图片路径> [更多图片...]\n".data(using: .utf8)!)
    exit(2)
}
...
for path in paths {
    ...
    do {
        ...
        let message = "识别失败 \(path): \(error)\n"
            + "Vision 需要访问系统服务，在受限沙箱里会失败；换到普通终端重跑，或让用户直接描述图片内容。\n"
